...
- Player Heatmaps overlay (enable/disable independently of trails) with configurable influence radius, grid cell size, opacity, and green→red gradient colors. Adjacent same-color cells are merged into larger rectangles (with a configurable height tolerance) to keep the web client fast even at small cell sizes
